"""
Port of src/context.ts — builds the system prompt and per-client context block
that is prepended to every Claude API call.
"""
...
_FCA = {"lower": 0.02, "intermediate": 0.05, "higher": 0.08}
_DEF = {"inflationRate": 0.02, "statePensionAge": 67, "statePensionWeekly": 221.20, "targetEndAge": 100}
_TIERS = [
    {"upTo": 25_000,        "rate": 0.005},
    {"upTo": 100_000,       "rate": 0.0023},
    {"upTo": 250_000,       "rate": 0.0018},
    {"upTo": 500_000,       "rate": 0.0013},
    {"upTo": float("inf"),  "rate": 0.0005},
]
_FUNDS = {"vanguardLifestrategy": 0.0022, "fidelityWorldTracker": 0.0012, "fidelityCashFund": 0.0010}
_ADVISER_CHARGE = 0.006


def build_client_context(client: dict) -> str:
    fact_find = client.get("fact_find") or client.get("factFind") or {}

    fact_find_lines = "\n".join(
        f"{cat}: {fact_find.get(cat, '[not yet collected]')}"
        for cat in FACT_FIND_CATEGORIES
    )

    extra_keys = [k for k in fact_find if k not in FACT_FIND_CATEGORIES]
    extra_lines = "\n".join(f"{k}: {fact_find[k]}" for k in extra_keys)

    recs = client.get("confirmed_recommendations") or client.get("confirmedRecommendations") or []
    if recs:
        recs_lines = "\n".join(
            f"[{r.get('confirmedAt', r.get('confirmed_at', ''))[:10]}] "
            f"{r['type']}: {r.get('summary', '')[:120]}"
            for r in recs
        )
    else:
        recs_lines = "None"

    open_items = client.get("open_items") or client.get("openItems") or []
    open_lines = "\n".join(f"- {item}" for item in open_items) if open_items else "None"

    MAX_SOFT = 8000
    soft = client.get("soft_knowledge") or client.get("softKnowledge") or ""
    trimmed_soft = ("[earlier sessions omitted]\n" + soft[-MAX_SOFT:]) if len(soft) > MAX_SOFT else soft
...
def _build_assumptions_block() -> str:
    def tier_label(t: dict) -> str:
        if t['upTo'] == float('inf'):
            return f"  Above £500,000: {t['rate'] * 100:.2f}%"
        return f"  Up to £{t['upTo']:,}: {t['rate'] * 100:.2f}%"

    tier_lines = "\n".join(tier_label(t) for t in _TIERS)
    inf = _DEF["inflationRate"]
    default_charges = 0.0108
    levels = [
        ("Lower (cautious)",       _FCA["lower"]),
        ("Intermediate (default)", _FCA["intermediate"]),
        ("Higher (adventurous)",   _FCA["higher"]),
    ]
    growth_lines = "\n".join(
        f"{name}: {gross * 100:.1f}% gross − {inf * 100}% inflation − "
        f"{default_charges * 100}% charges = {max(0.001, gross - inf - default_charges) * 100:.2f}% real net"
        for name, gross in levels
    )
    return f"""
=== UK TAX CONTEXT (2024/25) ===
Personal Allowance: £{_TAX['personalAllowance']:,}
Personal Allowance taper threshold: £{_TAX['personalAllowanceTaperThreshold']:,}
Basic Rate Band: up to £{_TAX['basicRateLimit']:,} ({int(_TAX['basicRate'] * 100)}%)
Higher Rate Band: up to £{_TAX['higherRateLimit']:,} ({int(_TAX['higherRate'] * 100)}%)
Additional Rate: {int(_TAX['additionalRate'] * 100)}%
CGT Annual Allowance: £{_TAX['cgtAllowance']:,}
CGT Rates: Basic {int(_TAX['cgtBasicRate'] * 100)}% / Higher {int(_TAX['cgtHigherRate'] * 100)}%
Dividend Allowance: £{_TAX['dividendAllowance']}

=== FCA STANDARD ILLUSTRATION GROWTH RATES ===
Formula: realNetGrowthRate = grossRate − inflation ({inf * 100}%) − totalCharges
...
=== DEFAULT ASSUMPTIONS ===
Inflation: {inf * 100}%
State Pension Age: {_DEF['statePensionAge']}
Full New State Pension: £{_DEF['statePensionWeekly']}/week (£{_DEF['statePensionWeekly'] * 52:.0f}/year)
Default projection end age: {_DEF['targetEndAge']}
...
Known fund OCFs:
  Vanguard LifeStrategy: {_FUNDS['vanguardLifestrategy'] * 100:.2f}%
  Fidelity World Tracker: {_FUNDS['fidelityWorldTracker'] * 100:.2f}%
  Fidelity Cash Fund: {_FUNDS['fidelityCashFund'] * 100:.2f}%

Default Adviser Charge: {_ADVISER_CHARGE * 100:.1f}% ongoing
...
GIA:           Subject to CGT on gains (after £{_TAX['cgtAllowance']:,} allowance). Dividends taxed as income. Key: "GIA"
...
def _build_output_format_instructions() -> str:
    categories_list = "\n".join(f"- {c}" for c in FACT_FIND_CATEGORIES)
    return f"""
...
- retainedAssets: anything the client keeps that is NOT being transferred or restructured
- phases: use for any time-sequenced actions beyond the immediate recommendation
...
# Build SYSTEM_PROMPT once at import time (it's static — no client data)
SYSTEM_PROMPT = (
    """You are an AI assistant embedded in the workflow of a regulated UK financial adviser. You work exclusively with the adviser, not clients. You operate with the reasoning capability of a senior financial planner but you are not the regulated adviser — nothing is confirmed or enacted without explicit adviser sign-off.
...
5. When asked for a scheme assessment, use all ingested documents to produce the scheme_assessment artifact"""
    ...
    + _build_assumptions_block()
    + _build_fund_reference()
)
